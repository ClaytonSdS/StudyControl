...
class Cronograma(Screen):
	...
	def EstudadoFunction_Home(self, root):
		...
		if str(self.ids.cronograma22.children[-1])[0:37] != validation_tabel and self.ids.cronograma22.children[-1].get_row_checks() != [] and len(self.ids.cronograma22.children[-1].get_row_checks()) > 0:
			...
			def Estudado(lista, select, color_hex='686fa3'):
				...
				for w in range(len(data_sem_duracao)):
					data_sem_duracao[w].remove(data_sem_duracao[w][5])
				...
				for x in range(len(step22)):
					if step22[x][-1][2] == '[color=#{}]Estudado[/color]'.format(color_hex):
						pass

					else:
						step22[x][-1][0] = 'checkbox-marked-circle'
						step22[x][-1][1] = [39 / 256, 174 / 256, 96 / 256, 1]
						step22[x][-1][2] = '[color=#{}]Estudado[/color]'.format(color_hex)
			...
			def count_Estudado(state):
				...
				for x in range(len(data_v)):
					if data_v[x][0] == str(today):
						if data_v[x][-1][2] == 'Estudado':
							count_estudado += 1

				for x in range(len(data_v)):
					if data_v[x][0] == str(today):
						if data_v[x][-1][2] == 'Não Estudado':
							count_naoestudado += 1
			...
			data_tables = MDDataTable(
				...
				elevation=0,
				check=True,
				rows_num=len(AutoSizeTableRowsNum(data_new)),
			...
			Clock.schedule_once(refreshChecks, 2)
	...
	def NaoEstudadoFunction_Home(self, root):
		...
		if str(self.ids.cronograma22.children[-1])[0:37] != validation_tabel and self.ids.cronograma22.children[
			...
			def NaoEstudado(lista, select, color_hex='686fa3'):
				...
				# COLOCAR ESTUDADO EM TODOS OS ITEMS DA LISTA SELECT
				for x in range(len(step22)):
					if step22[x][-1][2] == '[color=#{}]Não Estudado[/color]'.format(color_hex):
						pass

					else:
						step22[x][-1][0] = 'alert-circle'
						step22[x][-1][1] = [1,0,0, 1]
						step22[x][-1][2] = '[color=#{}]Não Estudado[/color]'.format(color_hex)
			...
			data_cronograma = colorData(organizerData(sortData(data_cronograma)))
			estudadoList = rebuildDataStyle(options)
			data_cronograma = NaoEstudado(data_cronograma, estudadoList)
	...
	def on_enter(self, *args):
		...
		Clock.schedule_once(adicionar_Tabela, 2)
		Clock.schedule_once(remover_spin, 2.05)
