...
class Cronograma(Screen):
		...
		if str(self.ids.cronograma22.children[-1])[0:37] != validation_tabel and self.ids.cronograma22.children[-1].get_row_checks() != [] and len(self.ids.cronograma22.children[-1].get_row_checks()) > 0:
			...
			try:
				...
				if len(DataSearched) >= 0:
					def AutoSizeTableRowsNum(lista, rows_per_page):
						lista = CronogramaTableStyle(colorData(organizerData(sortData(lista))))
						if len(lista) == 0: return createEmptyRow(createEmptyRow(lista))
						if len(lista) == 1: return createEmptyRow(lista)
						if len(lista) > 1:
							def sequence(rows_per_page, data):
								a = {}
								a[0] = rows_per_page
							...
							if len(lista) in divisores:
								...
								return lista
					#self.ids.cronograma22.children[0].update_row_data(self, AutoSizeTableRowsNum(DataSearched, grid.rows_per_page_cronograma + 1))
					try:
						...
						if len(estudados) == 0:
							...
							try:
								for i in range(1, num):
									self.ids.cronograma22.children[0].avancarPagina()
							except TypeError:
								pass
			...
			# NOVOS DADOS A PARTIR DA PAGINA INICIAL
				...
				try:
					...
					print('#########')
					print('num: ', num)
					print('estudados[0]: ', estudados[0])
					print('#########')
					if len(estudados) == 0:
						pass
					else:
						toast("Status Alterado Com Sucesso!")
						self.ids.cronograma22.children[0].update_row_data(self, AutoSizeTableRowsNum(DataSearched_today, grid.rows_per_page_cronograma + 1))
	...
	def on_enter(self, *args):
		...
		data_tables = MDDataTable(
			...
			column_data=[("[color=#686fa3]Data[/color]", dp(grid.tabela_cronograma_dp_data)), ("[color=#686fa3]Horário[/color]", dp(grid.tabela_cronograma_dp_horario)), ("[color=#686fa3]Categoria[/color]", dp(grid.tabela_cronograma_dp_categoria)), ("[color=#686fa3]Matéria[/color]", dp(grid.tabela_cronograma_dp_materia)),
						 ("[color=#686fa3]Tópico[/color]", dp(grid.tabela_cronograma_dp_topico)), ("[color=#686fa3]Status[/color]", dp(grid.tabela_cronograma_dp_status))],
			row_data= AutoSizeTableRowsNum(DataSearched_today, grid.rows_per_page_cronograma + 1)
		)

		#35

		#row_data= AutoSizeTableRowsNum_byPageNum(AutoSizeTableRowsNum(DataSearched_today), grid.rows_per_page_cronograma + 1)
...
class Rendimento(Screen):
	...
	def zoom_in_bar(self):
		if self.zoom_bar_plot < 8:
			self.zoom_bar_plot *= 2

	def zoom_out_bar(self):
		if self.zoom_bar_plot > 1:
			self.zoom_bar_plot /= 2
	...
	data = StringProperty(str(changeData(str(today))))

	# FUNÇÕES PARA ABRIR DIALOG E CRIAR UMA GRAFICO NO DIALOG
	def zoom_acertos_erros_graph (self, root, *args):
		data_cronograma = pickle.load((open("data_timeline.p", "rb")))
		today = date.today()

		if not self.acertos_erros_zoom_dialog:
			self.acertos_erros_zoom_dialog = MDDialog(
				title=f'[color={self.cor_aplicativo}][b]Acertos e Erros [/b]({self.data})[/color]',
				md_bg_color=(0.9019, 0.9215, 0.9843, 1),
				type="custom",
				auto_dismiss=True,
				content_cls=Zoom_acertos_erros()
				)
			# ALTERAR ACERTOS E ERROS DE ITENS FILTRADOS
			try:
				DataSearched = SearchByData(data_cronograma, str(self.data_selecionada))
				graph_bar_acertos_erros_functionPloter(self, DataSearched, str(self.data_selecionada), False)
			except AttributeError:
				DataSearched_today = SearchByData(data_cronograma, str(today))
				graph_bar_acertos_erros_functionPloter(self, DataSearched_today, str(self.today), False)

			self.acertos_erros_zoom_dialog.open()
			self.acertos_erros_zoom_dialog.on_dismiss = self.acertos_erros_dismiss

	def acertos_erros_dismiss(self):
		#self.ids.acertos_erros_bar_plot.remove_widget(self.ids.acertos_erros_bar_plot.children[0])
		self.acertos_erros_zoom_dialog.content_cls.ids.graph_acertos_erros.remove_widget(self.acertos_erros_zoom_dialog.content_cls.ids.graph_acertos_erros.children[0])
		self.acertos_erros_zoom_dialog.content_cls.ids.graph_acertos_erros.clear_widgets()
		self.acertos_erros_zoom_dialog = None
		del self.acertos_erros_zoom_dialog
	...
	def alterar_Acertos_Erros(self, root):
		...
		try:
			...
			self.ids.acertos_erros_bar_plot.remove_widget(self.ids.acertos_erros_bar_plot.children[0])
			graph_bar_acertos_erros_functionPloter(self, DataSearched, str(self.data_selecionada), True)
		...
		# ALTERAR ACERTOS E ERROS DA PÁGINA INICIAL
			...
			self.ids.acertos_erros_bar_plot.remove_widget(self.ids.acertos_erros_bar_plot.children[0])
			graph_bar_acertos_erros_functionPloter(self, DataSearched_today, str(self.today), True)
	...
	def on_enter(self, *args):
		...
		try:
			DataSearched = SearchByData(data_cronograma, str(self.data_selecionada))
			graph_bar_acertos_erros_functionPloter(self, data_cronograma, str(self.data_selecionada), True)
		except AttributeError:
			graph_bar_acertos_erros_functionPloter(self, data_cronograma, self.today, True)
