	...
	for num_page in range(1, len(dict_order.keys()) + 1):
		for index in range(len(dict_order[num_page])):
			if dict_order[num_page][index] == name_to_search:
				#print('achei em dict_order[{}][{}]'.format(num_page, index))
				return int(num_page)
...
class Cronograma(Screen):
	...
	# DIALOGO PARA CONFIRMAR SE DESEJA APAGAR MATERIA ESPECIFICA
	def materias_estudadas_dialog(self, *args):
		...
		if not self.dialog10:
			self.dialog10 = MDDialog(
				...
				md_bg_color=(0.9019, 0.9215, 0.9843, 1),
				type="custom",
				content_cls=Counteudo_Cronograma_MateriasEstudadas(),
				auto_dismiss=True,
				buttons=[
					MDFlatButton(
						text="[color=686fa3][b]CANCELAR[/color][/b]",
						theme_text_color="Custom",
						on_release=self.close_dialog10
					)]
			)
	...
	def close_confirmar_remover(self, obj):
		print(self.confirmar_remover.text)
	...
	def EstudadoFunction_Home(self, root):
		...
		if str(self.ids.cronograma22.children[-1])[0:37] != validation_tabel and self.ids.cronograma22.children[-1].get_row_checks() != [] and len(self.ids.cronograma22.children[-1].get_row_checks()) > 0:
			...
			# NOVOS DADOS A PARTIR DA PAGINA INICIAL
				...
				DataSearched_today = SearchByData(data_cronograma, str(today))
				#self.ids.cronograma22.children[0].update_row_data(self, AutoSizeTableRowsNum_byPageNum(AutoSizeTableRowsNum(DataSearched_today), grid.rows_per_page_cronograma + 1))
				...
				if lista_data[0] not in lista_data_to_check:
					...
					self.data_cronograma.append(
						(str(self.saveData[0]), str(self.saveTime[0]), root.ids.spinnerCategorias.text,
						 ...
						 ("alert-circle", [1, 0, 0, 1], "Não Estudado"), ' ', ' ', ' '))
...
class grid(MDApp):
	font_size_title_1 = 42
	font_size_title_2 = 30
	font_size_title_3 = 23

	font_size_normal_1 = 18
	font_size_normal_2 = 15
	...
	tabela_cronograma_dp_data = Window.size[0]*0.043
	tabela_cronograma_dp_horario = Window.size[0]*0.030
	tabela_cronograma_dp_categoria = Window.size[0]*0.030
	tabela_cronograma_dp_materia = Window.size[0]*0.032
	tabela_cronograma_dp_topico = Window.size[0]*0.039
	tabela_cronograma_dp_status = Window.size[0]*0.039
	# //////////////////////////////////////////////////////////////////////////
