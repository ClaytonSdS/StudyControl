...
def DataGreaterThanToday(lista):
  ...
  for h in range(len(step8)):
    ...
    if str(step8[h][0][18:20]) >= today[3:5] and str(step8[h][0][21:25]) == today[6:10]:
...
class Cronograma(Screen):
	...
	def on_enter(self, *args):
		...
		def AutoSizeTableRowsNum(lista):
			if len(lista) == 0:
				return createEmptyRowRemoveCronograma(createEmptyRowRemoveCronograma(DataGreaterThanToday(CronogramaTableStyle(colorData(organizerData(sortData(lista)))))))

			if len(lista) == 1:
				return createEmptyRowRemoveCronograma(DataGreaterThanToday(CronogramaTableStyle(colorData(organizerData(sortData(lista))))))

			if len(lista) > 1:
				return DataGreaterThanToday(CronogramaTableStyle(colorData(organizerData(sortData(lista)))))
		...
		Clock.schedule_once(my_callback2, 2)
...
class Cronograma_RemoverItem(Screen):
	...
	def search_by_string(self):
		...
		if len(DataByName) != 0:
			...
			data_DataByName = MDDataTable(
				...
				elevation=0,
				check=True,
				rows_num=len(createEmptyRowRemoveCronograma(
					createEmptyRowRemoveCronograma(DataByName))),
	...
	def on_enter(self, *args):
		...
		data_tables2 = MDDataTable(
			...
			elevation=0,
			check=True,
			rows_num =  len(createEmptyRowRemoveCronograma(createEmptyRowRemoveCronograma(colorData(organizerData(sortData(data_cronograma)))))),
			column_data=[("[color=#686fa3]Data[/color]", dp(35)), ("[color=#686fa3]Horário[/color]", dp(35)), ("[color=#686fa3]Categoria[/color]", dp(35)), ("[color=#686fa3]Matéria[/color]", dp(35)),
		...
		# createEmptyRowRemoveCronograma(createEmptyRowRemoveCronograma(colorData(organizerData(sortData(data_cronograma)))))
...
class Cronograma_AlterarStatus(Screen):
	...
	def EstudadoFunction(self, root):
		...
		if str(self.ids.cronogramaAllItems.children[-1])[0:37] != validation_tabel and self.ids.cronogramaAllItems.children[-1].get_row_checks() != [] and len(self.ids.cronogramaAllItems.children[-1].get_row_checks()) > 0:
			...
			pickle.dump(data_cronograma, open("data_timeline.p", "wb"))
			self.ids.cronogramaAllItems.children[0].caxa()
	...
	def NaoEstudadoFunction(self, root):
		...
		# IDENTIFICAR TABELA E ROWS CHECK DIFERENTE DE LISTA VAZIA
		if str(self.ids.cronogramaAllItems.children[-1])[0:37] != validation_tabel and \
				self.ids.cronogramaAllItems.children[-1].get_row_checks() != [] and len(
				self.ids.cronogramaAllItems.children[-1].get_row_checks()) > 0:
			# print('entrei funcao estudadofunction comprimento lista get_row_checks = ', len(self.ids.cronogramaAllItems.children[-1].get_row_checks()))
			...
			data_cronograma = colorData(organizerData(sortData(data_cronograma)))
			estudadoList = rebuildDataStyle(list(root.ids.cronogramaAllItems.children[-1].get_row_checks()))
	...
	def on_enter(self, *args):
		...
		def adicionarTabela(dt):
			self.ids.cronogramaAllItems.add_widget(data_tables2)
			print('\ndados antes de entrar na função = ', self.ids.cronogramaAllItems.children[0].get_row_checks())
