...
class TableData(RecycleView):
    ...
    def set_next_row_data_parts(self, direction: str) -> NoReturn:
        ...
        if direction == "reset":
            ...
            # REFRESH NOS CHECK AO AVANÇAR PARA PRÓXIMA PAGINA
            if self.check == True:
                self.select_all("down")
                def refreshChecks(dt):
                    self.select_all("normal")
                #Clock.schedule_once(refreshChecks, 0.05)
                Clock.schedule_once(refreshChecks, 0.05)
            ...
            # REFRESH NOS CHECK AO RECUAR PARA PAGINA ANTERIOR
            if self.check == True:
                self.select_all("down")
                def refreshChecks(dt):
                    self.select_all("normal")
...
class MDDataTable(ThemableBehavior, AnchorLayout):
    ...
    background_color = ColorProperty([0.9333,0.9490,0.9961, 1])
    ...
    def DoubleClickRefreshChecks (self):
        TableData.select_all(self.table_data, "down")
        TableData.select_all(self.table_data, "normal")
        TableData.select_all(self.table_data, "down")
        TableData.select_all(self.table_data, "normal")

    def avancarPagina (self):
        #TableData.table_data.set_next_row_data_parts("forward")
        TableData.set_next_row_data_parts(self.table_data, "forward")
