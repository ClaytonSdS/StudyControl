		...
		if self.acertos != "" and self.erros != "":
			...
			try:
				...
				self.ids.cronograma22.children[0].children[0].update_row_data(self, AutoSizeTableRowsNum_Cronograma(
					DataSearched, grid.rows_per_page_cronograma + 1))
	...
	class Tabela_Cronograma(MDBoxLayout):
		def __init__(self,**kwargs):
			...
			data_table_cronograma = MDDataTable(
				...
				column_data=[(f"[color=#{self.tema[0].cor_aplicativo}]Data[/color]", dp(grid.tabela_cronograma_dp_data - 2)),
							 (f"[color=#{self.tema[0].cor_aplicativo}]Horário[/color]", dp(grid.tabela_cronograma_dp_horario - 4)),
							 (f"[color={self.tema[0].cor_aplicativo}]Categoria[/color]", dp(grid.tabela_cronograma_dp_categoria) - 3),
